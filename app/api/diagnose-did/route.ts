...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("D-ID Auth Response:", authResponseText)

    // Step 4: Try a minimal talk creation request
    if ([200, 401, 403].includes(authResponse.status)) {
      console.log("Authentication seems to work, trying minimal request...")
      ...
      console.log("D-ID Create Response:", createResponseText)
      ...
      if (!createResponse.ok) {
        diagnostics.errors.push(`Minimal request failed: ${createResponse.status} - ${createResponseText}`)
      }
    } else {
